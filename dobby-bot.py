...
def search_date_pattern(string_to_search):
    ...
    rem_date = datetime.now() + timedelta(hours=tz_delta)
    dic.clear()
    ...
    for key, value in re_patterns.items():
        ...
        for item in re_v:
            ...
            if 'time' in key:
                ...
                rem_date = rem_date + relativedelta(months=1) if (datetime.now()+ timedelta(hours=tz_delta)) > rem_date else rem_date
...
def extract_date(chat_message):
    ...
    try:
        ...
        try:
            ...
            if (datetime.now() + timedelta(hours=tz_delta)) > remind_at:
